function [] = seegLA_beh_AVEpowerBASE_summary_v(behDIR , ephysDIR, bandOfInt , epoch1, epoch2, wire, channS, titLE)
...
for ggi = 1:2
    ...
    baselineV = mean(squeeze(avePower(1,bandOfInt,trial2use,:)),"all");
...
%         'Ch ' , num2str(chanIDtab.Chanl(ci))];
    ...
    if pval < 0.05
        title(['p = ' , num2str(round(pval, 3))], 'Color',[1 0 0],'FontWeight','bold')
    else
        title(['p = ' , num2str(round(pval, 3))],'Color',[0 0 0])
    end
